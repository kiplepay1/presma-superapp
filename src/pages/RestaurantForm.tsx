...
const emptyForm: NewRestaurantInput = {
  ...
  assignedBD: "", assignedOps: "", assignedTech: "", notes: "", surveyUrl: "", logoUrl: "", meetingPhotoUrls: [],
};
...
export default function RestaurantForm({
  ...
  const [form, setForm] = useState<NewRestaurantInput>(
    initial ? {
      ...
      surveyUrl: initial.surveyUrl ?? "", logoUrl: initial.logoUrl ?? "", meetingPhotoUrls: initial.meetingPhotoUrls ?? [],
    } : { ...emptyForm, ...prefill }
  );
  const [meetingPhotoInput, setMeetingPhotoInput] = useState("");
  ...
  const addMeetingPhoto = () => {
    const v = meetingPhotoInput.trim();
    if (v && !form.meetingPhotoUrls.includes(v)) set("meetingPhotoUrls", [...form.meetingPhotoUrls, v]);
    setMeetingPhotoInput("");
  };
  const removeMeetingPhoto = (url: string) => set("meetingPhotoUrls", form.meetingPhotoUrls.filter((u) => u !== url));
      ...
      <div className="max-h-[90vh] w-full max-w-lg overflow-y-auto rounded-md border border-border bg-surface shadow-pop" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="space-y-4 px-5 py-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              <label className={labelCls}>Survey Meeting Photo Link(s) <span className="text-ink-faint">(add as many as needed)</span></label>
              <div className="flex gap-2">
                <input
                  className={inputCls} value={meetingPhotoInput} onChange={(e) => setMeetingPhotoInput(e.target.value)}
                  onKeyDown={(e) => { if (e.key === "Enter") { e.preventDefault(); addMeetingPhoto(); } }}
                  placeholder="https://...jpg — press Enter"
                />
                <button type="button" onClick={addMeetingPhoto} className="shrink-0 rounded border border-border px-3 py-1.5 text-xs font-medium text-ink-soft hover:bg-paper">Add</button>
              </div>
              {form.meetingPhotoUrls.length > 0 && (
                <div className="mt-2 flex flex-wrap gap-1.5">
                  {form.meetingPhotoUrls.map((u, i) => (
                    <span key={u} className="flex max-w-full items-center gap-1 rounded-full bg-brand-50 px-2.5 py-1 text-xs text-brand-700">
                      <span className="max-w-[160px] truncate">Photo {i + 1}</span>
                      <button type="button" onClick={() => removeMeetingPhoto(u)} className="shrink-0 hover:text-status-red"><X size={12} /></button>
                    </span>
                  ))}
                </div>
              )}
