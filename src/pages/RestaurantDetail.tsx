...
export default function RestaurantDetail() {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="rounded border border-border bg-surface p-5 shadow-card">
        <div className="flex items-start justify-between">
          <div className="flex items-start gap-3">
            {r.logoUrl ? (
              <img src={r.logoUrl} alt="" className="h-12 w-12 shrink-0 rounded border border-border object-cover" onError={(e) => { (e.target as HTMLImageElement).style.display = "none"; }} />
            ) : (
              <div className="flex h-12 w-12 shrink-0 items-center justify-center rounded border border-border bg-paper text-xs text-ink-faint">Logo</div>
            )}
            <div>
      ...
      <SectionCard title="Restaurant Survey">
        {r.surveyUrl ? (
          <a href={r.surveyUrl} target="_blank" rel="noreferrer" className="flex w-fit items-center gap-2 rounded border border-border bg-paper px-3 py-2 text-sm text-brand-700 hover:border-brand-500">
            <FileText size={16} /> View Survey Document (PDF) <ExternalLink size={13} className="text-ink-faint" />
          </a>
        ) : (
          <p className="text-sm text-ink-faint">No survey document linked yet. Click Edit above to add a link once the survey PDF is uploaded (e.g. to OneDrive).</p>
        )}

        <div className="mt-4 grid grid-cols-2 gap-3 sm:grid-cols-4">
          <PhotoTile label="Logo" url={r.logoUrl} />
          {r.meetingPhotoUrls.length === 0 ? (
            <PhotoTile label="Survey Meeting Photo" url="" />
          ) : (
            r.meetingPhotoUrls.map((url, i) => (
              <PhotoTile key={url} label={i === 0 ? `Survey Meeting Photos (${r.meetingPhotoUrls.length})` : ""} url={url} />
            ))
          )}
        </div>
      </SectionCard>
...
function PhotoTile({ label, url }: { label?: string; url: string }) {
  ...
  return (
    <div>
      <div className="mb-1 h-4 text-xs font-medium text-ink-soft">{label}</div>
      {url && !failed ? (
        <a href={url} target="_blank" rel="noreferrer" className="flex h-32 w-full items-center justify-center overflow-hidden rounded border border-border bg-paper">
          <img src={url} alt={label || "Photo"} className="h-full w-full object-contain" onError={() => setFailed(true)} />
        </a>
      ) : (
        <div className="flex h-32 w-full items-center justify-center rounded border border-dashed border-border text-xs text-ink-faint">
          {url && failed ? "Couldn't load image — check the link" : "Not added yet"}
        </div>
      )}
    </div>
  );
